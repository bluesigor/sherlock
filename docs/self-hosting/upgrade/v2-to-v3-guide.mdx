...
<Note>This is a historical Sourcebot migration guide, retained as reference for its inherited storage format. Sherlock starts from v3.0.4 and has no v2 release.</Note>
...
If your deployment is dependent on these features, please [reach out](/docs/connections/request-new).
</Warning>
...
<Steps>
    ...
    <Step title="Delete Sherlock cache (.sourcebot directory)">
    </Step>
    <Step title="Migrate your configuration file to the v3 schema">
        ...
        - The name of the connection (`sourcebot-connection` and `wireshark-connection` above) is only used to identify the connection in Sherlock. It can be any string that contains letters, digits, hyphens, or underscores
    ...
    <Step title="Start your Sherlock deployment">
        When you start up your Sherlock deployment, it will create a fresh cache and begin indexing against your new v3 configuration file.

        If there are issues with your configuration file it will provide an error in the console.
        After updating your configuration file, restart your Sherlock deployment to pick up the new changes.
    </Step>
    <Step title="You're done!">
        Congrats, you've successfully migrated to v3! Please let us know what you think of the new features by reaching out to your [instance administrator](/docs/connections/request-new)
    </Step>
</Steps>
...
Having troubles migrating from v2 to v3? Reach out to us to your [instance administrator](/docs/connections/request-new) and we'll try our best to help
